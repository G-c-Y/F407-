...
#include "cmsis_os2.h"
uint8_t test_temperture_jump = 0;
...
{
    static float simulated_temperature = 10.0f;  //模拟温度值
    //参数检查
    if (update == NULL)
    {
        return 0U;
    ...
    //使用模拟数据（当没有真实传感器时使用）
    if(test_temperture_jump  == 0)
    {
        update->temperature = simulated_temperature;  //模拟温度
        update->humidity = simulated_temperature;//60.0f;         //固定湿度值
        update->temperture_valid = 1.0f;            //温度有效
        update->humidity_valid = 1.0f;             //湿度有效
       
    }
    else if(test_temperture_jump  == 1){ 
         update->temperature = 50.0f;
         update->humidity = 6.0f;     
    }
   

    //模拟温度变化（25°C -> 30°C 循环）
    simulated_temperature += 0.4f;
    if (simulated_temperature > 50.0f)
    {
